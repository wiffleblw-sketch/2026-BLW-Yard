...
function applyPA(game, state, ev) {
  ...
  if (result === 'HR') {
    ...
    if (before['3B'] && before['3B'] !== batterId) { scorers.push(before['3B']); runsThisPA++; }
    if (before['2B'] && before['2B'] !== batterId) { scorers.push(before['2B']); runsThisPA++; }
    if (before['1B'] && before['1B'] !== batterId) { scorers.push(before['1B']); runsThisPA++; }
  ...
  // If the event has more RBI than the engine inferred scorers, force-score additional runners.
  // This handles cases like a 2-RBI single where the engine's default doesn't score the 2B runner.
  if (ev.rbi && ev.rbi > scorers.length && result !== 'HR') {
    // Pull additional scorers from the highest occupied base downward.
    // (For HR we already score everyone, so skip.)
    const extraNeeded = ev.rbi - scorers.length;
    let added = 0;
    for (const base of ['3B', '2B', '1B']) {
      if (added >= extraNeeded) break;
      const runnerId = state.bases[base];
      if (!runnerId) continue;
      // Don't double-score someone already in scorers
      if (scorers.includes(runnerId)) continue;
...
export function finalizeToSeasonRecords(game) {
  ...
  const pas = [];
  // Replay to determine inning/half at time of each PA, and which players scored on each PA
  const replayState = {
  ...
  for (const ev of game.events) {
    if (ev.type === 'pa') {
      // If the PA event has an intended inning/half (from paste-game parser),
      // force the replay state to match BEFORE applying the event. This handles
      // the case where the user's notes had fewer than 3 outs per half.
      if (ev.intendedInning && ev.intendedHalf) {
        if (replayState.inning !== ev.intendedInning || replayState.half !== ev.intendedHalf) {
          // Advance halves until we match.
          let safety = 0;
          while ((replayState.inning < ev.intendedInning ||
                 (replayState.inning === ev.intendedInning && replayState.half === 'T' && ev.intendedHalf === 'B'))
                 && safety++ < 20) {
            // Force-advance by emitting an end_half-equivalent state mutation
            replayState.outs = 3;
            applyEvent(game, replayState, { type: 'end_half', id: 'force', ts: 0 });
          }
        }
      }
      const inning = replayState.inning;
      const half = replayState.half;
      // Apply the event so we know who scored on this PA
      applyEvent(game, replayState, ev);
      const scorersOnThisPA = replayState.lastResult?.scorers || [];
      pas.push({
        ...
        result: ev.result,
        rbi: ev.rbi || 0,
        runs: scorersOnThisPA.length,
        runners_scored: scorersOnThisPA,
        notes: ev.notes || '',
      });
    } else {
      applyEvent(game, replayState, ev);
