...
export function finalizeToSeasonRecords(game) {
  ...
  for (const ev of game.events) {
    if (ev.type === 'pa') {
      ...
      // runs = total runners who scored on this PA (used for team/inning/score totals)
      // runners_scored = array of player IDs (used to attribute individual runs-scored stats)
      pas.push({
