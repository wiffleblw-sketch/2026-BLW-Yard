...
// Levenshtein distance — quick approximate string match (cap at 2 for early exit)
...
function matchPlayer(nameStr, roster) {
  ...
  // Fuzzy matching (typo tolerance) — last resort
  // Try last name with up to 2 char edits
  const fuzzy = roster.map(p => {
    const parts = p.name.split(/\s+/);
    const last = normalizeStr(parts[parts.length - 1]);
    const first = normalizeStr(parts[0]);
    const full = normalizeStr(p.name);
    const dLast = editDistance(q, last, 2);
    const dFirst = editDistance(q, first, 2);
    const dFull = editDistance(q, full, 2);
    return { player: p, dist: Math.min(dLast, dFirst, dFull) };
  }).filter(x => x.dist <= 2).sort((a, b) => a.dist - b.dist);

  if (fuzzy.length >= 1 && fuzzy[0].dist <= 2) {
    // If there's a clear winner (the second-best is significantly worse), take it
    if (fuzzy.length === 1 || fuzzy[1].dist > fuzzy[0].dist) {
      return { player: fuzzy[0].player, ambiguous: false, fuzzy: true };
    }
    return { player: fuzzy[0].player, ambiguous: true, candidates: fuzzy.map(x => x.player) };
  }
...
export function parseGameScript({
  ...
  let awayStarterId = null;          // first away pitcher seen
  let homeStarterId = null;          // first home pitcher seen
  ...
  for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
    ...
    if (pitcherName) {
      if (!currentFieldingRoster) {
        ...
        if (!awayCurrentPitcher && !homeCurrentPitcher) {
          ...
          if (match) {
            ...
            if (!homeStarterId) homeStarterId = match.player.id;
            continue;
          }
          const match2 = matchPlayer(pitcherName, awayRoster);
          if (match2) {
            awayCurrentPitcher = match2.player.id;
            if (!awayStarterId) awayStarterId = match2.player.id;
            continue;
          }
      ...
      if (side === 'home') {
        ...
        awayCurrentPitcher = match.player.id;
        if (!awayStarterId) awayStarterId = match.player.id;
      }
    ...
    if (pa) {
      ...
      const ev = {
        ...
        // Authoritative inning/half from the parsed script — used by finalize replay
        // to override the engine's auto-half advance when the user's notes have
        // fewer than 3 outs per half.
        intendedInning: currentInning,
        intendedHalf: currentHalf,
        ts: events.length,
      };
      events.push(ev);
  ...
  // Prefer explicitly-tracked starters
  const awayStarter = awayStarterId || awayCurrentPitcher || null;
  const homeStarter = homeStarterId || homeCurrentPitcher || null;
